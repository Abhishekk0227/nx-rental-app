...
export default function BookedVehicles({
  ...
  const getBookingFinancialSnapshot = (booking, baseFareOverride, depositOverride, advancePaidOverride, extensionsOverride, dropDetailsOverride, depositDetailsOverride, additionalPayment, paymentCollectionOverride) => {
    ...
    const addonsTotal = ((booking.addons?.helmetsCount > 1 ? booking.addons.helmetsCount - 1 : 0) * (booking.addons?.helmetsPrice || 50));
  ...
  const [replacePaymentMode, setReplacePaymentMode] = useState('');
  ...
  const openEdit = (booking) => {
    ...
    setEditPickupDate(formatLocalISO(new Date(booking.pickupDate || booking.rentalPeriod?.startDate)));
    setEditExpectedDropDate(formatLocalISO(new Date(booking.expectedDropDate || booking.expectedReturnDate || booking.rentalPeriod?.expectedEndDate)));
    setEditPlanType(booking.selectedPlan?.planType || '24-Hour');
    setEditSecurityDeposit(booking.securityDeposit || 0);
    setEditStartKm(booking.handover?.startMeter || booking.pickupDetails?.odometerStart || 0);
  ...
  const getExtendDepositComparison = () => {
    ...
    } else if (extensionPaymentMode === 'Vikas') {
      additionalVikas = additionalDeposit;
  ...
  const getReplacePricingComparison = () => {
    ...
    const totalExt = selectedBooking.extensions?.reduce((sum, ext) => sum + ext.extraCharges, 0) || 0;
  ...
  const handleExtendSubmit = (e) => {
    ...
    const oldDDetails = selectedBooking.depositDetails || {};
    const oldMode = oldDDetails.mode || 'Cash';
    const oldCash = Number(oldDDetails.cashAmount || (oldMode === 'Cash' ? selectedBooking.securityDeposit : 0));
    const oldOnline = Number(oldDDetails.onlineAmount || (['Online', 'UPI'].includes(oldMode) ? selectedBooking.securityDeposit : 0));
    const oldVikas = Number(oldDDetails.vikasAmount || (oldMode === 'Vikas' ? selectedBooking.securityDeposit : 0));
    ...
    if (Number(extensionAdditionalDeposit) > 0) {
      if (extensionPaymentMode === 'Cash') {
        ...
        addVikas = Number(extensionAdditionalDeposit);
      } else if (extensionPaymentMode === 'Mixed') {
    ...
    const newVikasDeposit = oldVikas + addVikas;
    const activeModes = [];
    if (newCashDeposit > 0) activeModes.push('Cash');
    if (newOnlineDeposit > 0) activeModes.push('Online');
    if (newVikasDeposit > 0) activeModes.push('Vikas');
    const newMode = activeModes.length > 1 ? 'Mixed' : (activeModes[0] || 'Cash');
    ...
    const payload = {
      newEndDateTime: new Date(extensionEndDate).toISOString(),
      ...
      baseFare: selectedBooking.baseFare + Number(extensionExtraCharges),
      securityDeposit: selectedBooking.securityDeposit + Number(extensionAdditionalDeposit),
      depositHeld: selectedBooking.securityDeposit + Number(extensionAdditionalDeposit),
      depositDetails: {
        mode: newMode,
        cashAmount: newCashDeposit,
        onlineAmount: newOnlineDeposit,
  ...
  const handleReplaceSubmit = (e) => {
    ...
    if (applyNewPricing && comp) {
      if (comp.totalDiff !== 0 && !replacePaymentMode) {
        return alert('Please select a Payment Mode for the settlement difference.');
      }
      const oldDDetails = selectedBooking.depositDetails || {};
      ...
      const oldVikas = Number(oldDDetails.vikasAmount || (oldDDetails.mode === 'Vikas' ? selectedBooking.securityDeposit : 0));
      ...
      if (comp.depositDiff > 0) {
        ...
        } else if (['UPI', 'Online', 'Card'].includes(replacePaymentMode)) {
          depOnlineAdd = comp.depositDiff;
        } else if (replacePaymentMode === 'Vikas') {
          depVikasAdd = comp.depositDiff;
      ...
      let newVikasDeposit = oldVikas + depVikasAdd;
      let newAdvancePaid = selectedBooking.advancePaid;
      let finalPayments = [...(selectedBooking.paymentCollection || [])];

      if (comp.depositDiff < 0) {
        const depositRefundAmt = Math.abs(comp.depositDiff);
        if (replacePaymentMode === 'Defer') {
          // If deferred, move the extra deposit to advancePaid so it gets refunded at drop-off
          newAdvancePaid += depositRefundAmt;
          // Keep the deposit held tracking accurate to the new required deposit
          if (newOnlineDeposit >= depositRefundAmt) {
            newOnlineDeposit -= depositRefundAmt;
          } else {
            newCashDeposit -= (depositRefundAmt - newOnlineDeposit);
            newOnlineDeposit = 0;
          }
        } else {
          // Instant refund
      ...
      } else if (comp.totalDiff < 0 && replacePaymentMode !== 'Defer') {
        if (replacePaymentMode === 'Cash' || ['UPI', 'Online', 'Card'].includes(replacePaymentMode)) {
          const refundAmt = Math.abs(comp.totalDiff);
          // Instead of subtracting from advancePaid (which breaks if refund > rent), 
          // we just log the refund transaction. The deposit part was already subtracted from deposit pools.
          // For the rent part, we subtract it from newAdvancePaid.
          const rentRefundAmt = Math.abs(comp.rentDiff < 0 ? comp.rentDiff : 0);
          newAdvancePaid = Math.max(0, newAdvancePaid - rentRefundAmt);

          finalPayments.push({
            mode: replacePaymentMode,
            amount: -refundAmt,
            transactionId: `TXN-REP-REFUND-${Math.floor(100000 + Math.random() * 900000)}`,
            reference: `Replacement Refund Upfront (${replacePaymentMode})`,
            timestamp: new Date().toISOString()
          });
        }
      }
      ...
      payload = {
        ...
        securityDeposit: comp.newDeposit,
        depositHeld: comp.newDeposit,
  ...
  const handleEditSubmit = (e) => {
    ...
    const pathsBooking = [
      'pickupDate', 'expectedDropDate', 'selectedPlan.planType', 'addons.helmetsCount', 'addons.otherAccessories', 'handover.startMeter'
    ];
    ...
    const payload = {
      ...
      outstandingRent: Math.max(0, (Number(editBaseFare) + ((Number(editHelmetsCount) > 1 ? Number(editHelmetsCount) - 1 : 0) * 50) - Number(editDiscountAmount)) - Number(editAdvancePaid)),
      ...
      settlement: {
        ...
        remainingToPay: Math.max(0, (Number(editBaseFare) + ((Number(editHelmetsCount) > 1 ? Number(editHelmetsCount) - 1 : 0) * 50) - Number(editDiscountAmount)) - Number(editAdvancePaid))
      }
    };
  ...
  const triggerPrintDetails = (booking) => {
    ...
    const operatorName = users.find(u => u._id === booking.workerId)?.name || booking.workerId || 'System';
      ...
      <html>
        ...
        : '<div class="inv-row"><span class="label" style="color:#94a3b8;">No extensions</span><span class="value">—</span></div>'}
            ...
            <div class="inv-section">
              ...
              <table class="fin-table">
                ...
                <tbody>
                  <tr><td>Base Rental Fare</td><td>₹${fmtNum(snapshot.originalBaseFare)}</td></tr>
                  <tr><td>Extensions Total</td><td>₹${fmtNum(snapshot.extTotal)}</td></tr>
                  <tr><td>Add-ons Total</td><td>₹${fmtNum(snapshot.addonsTotal)}</td></tr>
                  ${booking.dropDetails ? `
                    <tr><td>Damage Charges</td><td>₹${fmtNum(booking.dropDetails.damageCharges)}</td></tr>
                    <tr><td>Cleaning Charges</td><td>₹${fmtNum(booking.dropDetails.cleaningCharges)}</td></tr>
                    <tr><td>Late / Extra Hours Charges</td><td>₹${fmtNum(booking.dropDetails.lateCharges)}</td></tr>
                    <tr><td>Other Charges</td><td>₹${fmtNum(booking.dropDetails.otherCharges)}</td></tr>
                  ` : ''}
                  <tr class="discount"><td>Discount</td><td>- ₹${fmtNum(snapshot.discount)}</td></tr>
                  <tr class="subtotal"><td>Total Rental Cost</td><td>₹${fmtNum(snapshot.rentalCost)}</td></tr>
                  <tr><td>Advance Paid</td><td style="color:#10b981;">- ₹${fmtNum(snapshot.rentalPaid)}</td></tr>
                  <tr><td>Security Deposit Held</td><td>₹${fmtNum(snapshot.depositHeld)}</td></tr>
                </tbody>
              </table>
            </div>
        ...
        : 'PENDING'}
            ...
            <div class="inv-footer">
              ...
              <div class="sub">Operator: ${operatorName}</div>
              <div class="terms">
        ...
        <>
          ...
          {showFilterBar && (
            <div className="glass-panel animate-fade" style={{ background: 'rgba(0,0,0,0.2)', padding: '16px', borderRadius: '8px', marginBottom: '16px', display: 'flex', flexDirection: 'column', gap: '14px' }}>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(280px, 1fr))', gap: '20px', borderTop: '1px solid rgba(255,255,255,0.05)', paddingTop: '12px' }}>
          ...
          {/* Cards Grid List — responsive layout */}
          <div>
            <div style={{ display: 'flex', flexDirection: 'column', gap: '14px' }}>
              ...
              {sortedBookings.map(b => {
                ...
                const expDate = b.expectedDropDate || b.expectedReturnDate || b.rentalPeriod?.expectedEndDate;
                const remainingTime = getRemainingTimeText(expDate, b.status);
                ...
                return (
                  ...
                  >
                    <div style={{ padding: '16px', display: 'flex', flexWrap: 'wrap', gap: '16px', justifyContent: 'space-between', alignItems: 'flex-start' }}>
                      {/* Left side matching Mockup 3 layout */}
                      <div style={{ display: 'flex', gap: '14px', flex: '1 1 min-content', alignItems: 'center' }}>
                        <div style={{
                          width: '60px',
                          height: '60px',
                          borderRadius: '8px',
                          ...
                          {(() => {
                            ...
                            const hasImage = resolvedV?.images?.front;
                            if (hasImage) {
                              return <img src={hasImage} alt="Vehicle" style={{ width: '100%', height: '100%', objectFit: 'cover' }} />;
                            }
                            return b.vehicleDetails?.category?.toLowerCase() === 'car' ? <Car size={28} color="#6366f1" /> : <Bike size={28} color="#6366f1" />;
                          })()}
                        </div>
                        ...
                        <div style={{ display: 'flex', flexDirection: 'column', gap: '4px' }}>
                          ...
                          <div style={{ fontSize: '0.75rem', color: 'var(--text-secondary)', display: 'flex', alignItems: 'center', gap: '6px', flexWrap: 'wrap' }}>
                            ...
                            <span style={{ display: 'inline-flex', alignItems: 'center', gap: '4px', color: '#6366f1', fontWeight: 600 }}>
                              <User size={12} /> {users.find(u => u._id === b.workerId)?.name || b.workerId || 'System'}
                            </span>
                          </div>
                        </div>
                      </div>
                    ...
                    <>
                      <div style={{ display: 'grid', gridTemplateColumns: '1.2fr 1fr', gap: '16px', marginBottom: '14px' }}>
                        ...
                        <div style={{
                          ...
                          <div>Scheduled End Time: <strong style={{ color: '#1e3a8a', marginLeft: '4px' }}>{formatDate(selectedBooking.expectedDropDate || selectedBooking.expectedReturnDate || selectedBooking.rentalPeriod?.expectedEndDate)}</strong></div>
                ...
                <div style={{ display: 'grid', gridTemplateColumns: '1.2fr 1fr', gap: '20px', fontSize: '0.85rem' }}>
                  ...
                  <div style={{ display: 'flex', flexDirection: 'column', gap: '6px', color: '#475569' }}>
                    ...
                    <div style={{ border: '1px solid #cbd5e1', padding: '14px', borderRadius: '8px', background: '#f8fafc' }}>
                      ...
                      <div style={{ color: '#2563eb', fontWeight: 'bold', fontSize: '1.1rem' }}>₹{((selectedBooking.addons?.helmetsCount > 1 ? selectedBooking.addons.helmetsCount - 1 : 0) * (selectedBooking.addons?.helmetsPrice || 50))}</div>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
              ...
              <div style={{
                ...
                {(() => {
                  ...
                  let origAddonsTotal = ((selectedBooking.addons?.helmetsCount > 1 ? selectedBooking.addons.helmetsCount - 1 : 0) * (selectedBooking.addons?.helmetsPrice || 50));
                  ...
                  if (firstRevision) {
                    ...
                    origAddonsTotal = ((Number(createHelmetsCount) > 1 ? Number(createHelmetsCount) - 1 : 0) * (selectedBooking.addons?.helmetsPrice || 50));
              ...
              {hasGenuineChanges && (
                <div style={{
                  ...
                  {(() => {
                    ...
                    const addonsTotal = ((selectedBooking.addons?.helmetsCount > 1 ? selectedBooking.addons.helmetsCount - 1 : 0) * (selectedBooking.addons?.helmetsPrice || 50));
                    ...
                    return (
                      <div style={{ fontSize: '0.85rem', display: 'flex', flexDirection: 'column', gap: '14px', color: '#475569' }}>
                        ...
                        <div style={{ background: '#f8fafc', border: '1px solid #cbd5e1', borderRadius: '8px', padding: '12px' }}>
                          ...
                          {totalPaidCollected > totalBookingValue && (
                            <div style={{ display: 'flex', justifyContent: 'space-between', margin: '4px 0', color: '#10b981', fontWeight: 'bold', fontSize: '0.8rem', background: 'rgba(16,185,129,0.08)', padding: '4px 6px', borderRadius: '4px' }}>
                              <span>➡️ Forwarded Swap Refund Credit:</span>
                              <span>+₹{totalPaidCollected - totalBookingValue}</span>
                            </div>
                          )}
                        </div>
            ...
            <div className="glass-panel animate-slide-up" style={{ width: '100%', maxWidth: '900px', margin: '0 auto' }}>
              ...
              <form onSubmit={handleExtendSubmit}>
                <div className="modal-body" style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
                  ...
                  <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '10px' }}>
                    <div className="form-group">
                      <label style={{ display: 'flex', alignItems: 'center', gap: '6px' }}>Current Duration <Lock size={12} color="#94a3b8" /></label>
                      <input type="text" className="form-control" style={{ backgroundColor: '#f8fafc', color: '#64748b', cursor: 'not-allowed', borderColor: '#e2e8f0' }} value={`${selectedBooking.durationHours || 24} hours`} disabled />
                    </div>
                    <div className="form-group">
                      <label style={{ display: 'flex', alignItems: 'center', gap: '6px' }}>Current Expected Return <Lock size={12} color="#94a3b8" /></label>
                      <input type="text" className="form-control" style={{ backgroundColor: '#f8fafc', color: '#64748b', cursor: 'not-allowed', borderColor: '#e2e8f0' }} value={isNaN(currentEnd.getTime()) ? 'N/A' : currentEnd.toLocaleString()} disabled />
                    </div>
                  </div>

                  <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '10px' }}>
                    <div className="form-group">
                      <label style={{ display: 'flex', alignItems: 'center', gap: '6px', color: '#3b82f6', fontWeight: 600 }}>New Return Date & Time <Pencil size={12} /></label>
                      <input
                        type="datetime-local"
                        className="form-control"
                        style={{ backgroundColor: '#fff', borderColor: '#3b82f6', boxShadow: '0 0 0 2px rgba(59, 130, 246, 0.1)' }}
                      ...
                      <small style={{ display: 'block', marginTop: '4px', fontSize: '0.7rem', color: '#64748b' }}>
                        Time format (12h AM/PM vs 24h) depends on your system settings.
                      </small>
                    </div>
                    <div className="form-group">
                      <label style={{ display: 'flex', alignItems: 'center', gap: '6px' }}>Additional Hours <Lock size={12} color="#94a3b8" /></label>
                      <input type="text" className="form-control" style={{ backgroundColor: '#f8fafc', color: '#64748b', cursor: 'not-allowed', borderColor: '#e2e8f0' }} value={`${additionalHours} Hour(s)`} disabled />
                    </div>
                  </div>
                      ...
                      {comp.totalDiff !== 0 && (
                        <div style={{ marginTop: '12px', display: 'flex', flexDirection: 'column', gap: '8px' }}>
                          <label style={{ fontWeight: 'bold' }}>
                            {comp.totalDiff < 0 ? 'Refund Settlement Preference' : 'Payment Mode for Extra Collection'}
                          </label>
                          <select
                            className="form-control"
                          ...
                          {comp.totalDiff < 0 && replacePaymentMode === 'Defer' && (
                            <div style={{ fontSize: '0.75rem', color: '#10b981', background: 'rgba(16,185,129,0.1)', padding: '8px 12px', borderRadius: '6px', marginTop: '4px' }}>
                              💡 ₹{Math.abs(comp.totalDiff)} refund will be automatically added to the return balance and paid back to the customer during Drop-off!
                            </div>
                          )}

                          {replacePaymentMode === 'Mixed' && comp.totalDiff > 0 && (
                ...
                <div className="modal-body" style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
                  ...
                  <div>
                    ...
                    <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '10px' }}>
                      ...
                      <div className="form-group">
                        <label>Helmet Counts</label>
                        <input type="text" inputMode="numeric" className="form-control" value={editHelmetsCount} onChange={e => setEditHelmetsCount(Number(e.target.value))} />
                      </div>
                      <div className="form-group" style={{ gridColumn: 'span 2' }}>
                      ...
                      <div className="form-group">
                        <label>Starting KM (Odometer at Pickup)</label>
                        <input type="text" inputMode="numeric" className="form-control" value={editStartKm} onChange={e => setEditStartKm(Number(e.target.value))} />
                      </div>
                    </div>
                  </div>
                  ...
                  <div>
                    ...
                    <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '10px' }}>
                      <div className="form-group">
                        <label>Rental Cost (Base Fare) (₹) *</label>
                        <input type="text" inputMode="numeric" className="form-control" value={editBaseFare} onChange={e => setEditBaseFare(Number(e.target.value))} required />
                      </div>
                      <div className="form-group">
                        <label>Discount Amount (₹)</label>
                        <input type="text" inputMode="numeric" className="form-control" value={editDiscountAmount} onChange={e => setEditDiscountAmount(Number(e.target.value))} />
                      </div>
                      <div className="form-group">
